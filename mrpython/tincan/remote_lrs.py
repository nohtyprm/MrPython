...
class RemoteLRS(Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "username" in kwargs \
            ...
            s = kwargs["username"] + ":" + kwargs["password"]

            '''
            auth_string = "Basic " + base64.b64encode(str(kwargs["username"]) +
                                                      ":" +
                                                      str(kwargs["password"]))
            auth_string = "Basic " + str(base64.b64encode((kwargs["username"]+":"+kwargs["password"]).encode()))[2:][:-1]
            '''
            auth_string = kwargs["username"] + ":" + kwargs["password"]
            #Encodage du string s en bytes, puis conversion en base 64
            auth_string = base64.b64encode(auth_string.encode())
            #Reconversion en string. Il faut le parametre 'utf-8' a str pour enlever les marqueurs de bytes "b'".
            auth_string ="Basic " + str(auth_string,'utf-8')
